...
sub extract_contact_links {
    ...
    my @potential_contact_links_by_url = grep { $_ =~ /$grep_url_expr/ } @$all_links;
    
    my @contact_links = ( @potential_contact_links_by_url, @potential_contact_links);
    @contact_links = uniq @contact_links;

    $self->{non_contact_links} = List::Compare->new($all_links, \@contact_links)->get_symdiff;
