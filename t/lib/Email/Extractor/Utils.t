...
subtest "looks_like_url" => sub {
    ...
    is 'http://fabmarkt.ru', looks_like_url('http://fabmarkt.ru'), 'http url return self';
    
    ok looks_like_url('https://fabmarkt.ru'), 'https url';
    is 'https://fabmarkt.ru', looks_like_url('https://fabmarkt.ru'), 'https url return self';
    
    ok looks_like_url('http://www.dns-shop.ru/'), 'http url with trailing slash';
    is 'http://www.dns-shop.ru/', looks_like_url('http://www.dns-shop.ru/'), 'http url with trailing return self';
    
    ok looks_like_url('https://www.dns-shop.ru/'), 'https url with trailing slash';
    is 'https://www.dns-shop.ru/', looks_like_url('https://www.dns-shop.ru/'), 'https url with trailing return self';
};
...
subtest "looks_like_file" => sub {
    ok looks_like_file('file:///htmls/regexp_test_1.html'), 'path with extension is file';
...
subtest "absolutize_links_array" => sub {
    ...
    my $b = [ 'http://example.com/contacts', 'http://example.com/about', 'http://example.com/test' ];
    my $dname = 'http://example.com';
    
    dies_ok { absolutize_links_array($a) } 'expecting to die when no dname provided';
...
my $html_chunks = [ '<a href="/test">Example</a>', '<a href="/test2">Example2</a>', '<a href="/lalala">Lalala</a>'];
my $links = [ '/test', '/test2', '/lalala' ];

subtest "find_all_links" => sub {
    is_deeply find_all_links( join('', @$html_chunks) ), $links, 'found all links';
};
...
subtest "find_links_by_text" => sub {
    ...
    is_deeply find_links_by_text( join('', @$html_chunks), 'Example2' ), [ $links->[1] ], 'all ok';
    
    is_deeply find_links_by_text($html_chunks->[0], 'Example'), [ $links->[0] ], 'one chunk ok';
    
    is_deeply 
        find_links_by_text($html_chunks->[0].''.$html_chunks->[1], 'Example'), 
        [ $links->[0] ], 
        'two chunks ok';
...
subtest "remove_query_params" => sub {
    my $res = $l; $res->[1] = 'http://example.com/js/html5.js';
    is_deeply remove_query_params($l), $res, 'Params removed and no query urls leave untouched';
};

subtest "drop_asset_links" => sub {
    is_deeply drop_asset_links($l), [ $l->[2], $l->[4] ], 'asset links droped, queried and no query both';
};
...
subtest "remove_external_links" => sub {
    ...
    dies_ok { remove_external_links($l) } 'expecting to die when no dname provided';
